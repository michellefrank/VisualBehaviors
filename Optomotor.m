function Optomotor(xv, twolines, screenid)
% MovingLineDemo([xv=10][, twolines=0][, screenid=max])
%
% Constructs a set of moving lines as an optomotor stimulus for fruit
% flies.
%
% Written by MMF 11/2015, modified from Psychtoolbox3 MovingLineDemo
%
...
while ~button(1)
    ...
    % gradient:
    Screen('DrawLines', win, [x, x ; 0, h], lw, [.4 0 .6]);
    if twolines
        Screen('DrawLines', win, [x+lw+xv, x+lw+xv ; 0, h], lw, [0.4, 0.6, 0]);
    end
